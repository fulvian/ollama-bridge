...
try:
    ...
    decision = s.get("routing_decision", routing)
    ...
    force_str = f" | FORCE={force}" if force else ""
    tier = "" if decision == "anthropic" else (" | TIER=haiku_flash" if decision == "haiku_to_flash" else " | TIER=full_ollama")
    print(f"[OllamaBridge] routing={routing} | 5h={pct5h:.1f}% | 7d={pct7d:.1f}% | model={model}{tier}{warn}{force_str}")
except Exception:
    pass
PYEOF
